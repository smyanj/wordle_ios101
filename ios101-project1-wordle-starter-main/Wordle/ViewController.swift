...
class ViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    /*
      Exercise 3(DONE): Assign a closure to the `didSelectString` property of `keyboardController` (see KeyboardController.swift):
    ...
    // START YOUR CODE HERE
      keyboardController.didSelectString = {(val1: String) -> () in
          if(val1 == "DEL"){
              self.boardController.deleteLastCharacter()
            }
            else{
                self.boardController.enter(val1)
            }
          }
